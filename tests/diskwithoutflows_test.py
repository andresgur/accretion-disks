...
from accretion_disks.diskwithoutflows import InnerDiskODE, InnerDisk
...
class TestDiskWithOutflows(unittest.TestCase):
    def testManualRsph(self):
        ...
        for diskclass in [InnerDiskODE, InnerDisk]:
            ...
            ss73 = ShakuraSunyaevDisk(
                ...
                Rmax=5000,
                Rmin=1.62 * mdot,
                Wrphi_in=disk.Wrphi[-1],
            )
            self.assertAlmostEqual(
                ss73.L() / blackhole.LEdd,
                1,
                delta=0.1,
                msg="Modified SS73 does not produce Eddington luminosity!",
            )
    ...
    def testODE(self):
        ...
        for mdot in np.arange(10, 100, 10):
            analyticalDisk = CompositeDisk(
                InnerDisk, ShakuraSunyaevDisk, blackhole, mdot=mdot, Rmax=1e5
            )
            odeDisk = CompositeDisk(
                InnerDiskODE, ShakuraSunyaevDisk, blackhole, mdot=mdot, Rmax=1e5
            )
            np.testing.assert_almost_equal(
                odeDisk.H[1:] / analyticalDisk.H[1:],
                np.ones_like(odeDisk.R[1:]),
                decimal=1,
            )
            np.testing.assert_almost_equal(
                odeDisk.Mdot[1:] / analyticalDisk.Mdot[1:],
                np.ones_like(odeDisk.R[1:]),
                decimal=1,
            )
            np.testing.assert_almost_equal(
                odeDisk.Wrphi[1:] / analyticalDisk.Wrphi[1:],
                np.ones_like(odeDisk.R[1:]),
                decimal=1,
            )
    ...
    def testDerivedRsphL(self):
        ...
        for diskclass in [InnerDisk, InnerDiskODE]:
            disk = CompositeDisk(
                diskclass, ShakuraSunyaevDisk, blackhole, mdot=mdot, N=5000, Rmax=1e5
            )
            ...
            self.assertAlmostEqual(
                ...
                1,
                delta=0.1,
                msg="L(R > Rsph) does not mach the Eddington luminosity!",
            )
            L = disk.outerDisk.L()
            self.assertAlmostEqual(
                L / blackhole.LEdd,
                1,
                delta=0.1,
                msg="The outer disk luminosity does not mach the Eddington luminosity!",
            )
    ...
    def testEnergyConserved(self):
        ...
        for diskclass in [InnerDisk, InnerDiskODE]:
            disk = CompositeDisk(
                diskclass, ShakuraSunyaevDisk, blackhole, mdot=mdot, Rmax=1e5, N=5000
            )
    ...
    def testAlphaRaises(self):
        ...
        with self.assertRaises(ValueError):
            ...
            # Wrphi >0
            CompositeDisk(
                CO=blackhole, mdot=0.1, alpha=0.1, Rmin=0.1, Rmax=5, Wrphi_in=1
            )
    ...
    def testQrad(self):
        ...
        for diskclass in [InnerDisk, InnerDiskODE]:
            disk = CompositeDisk(
                diskclass, ShakuraSunyaevDisk, blackhole, mdot=mdot, Rmax=1e4, N=10000
            )
            ...
            np.testing.assert_allclose(
                Qrad[rrange][1:-1] / disk.Qrad[rrange][1:-1],
                np.ones_like(Qrad[rrange])[1:-1],
                rtol=0.1,
                err_msg="Failure in Qrad calculation!",
            )
