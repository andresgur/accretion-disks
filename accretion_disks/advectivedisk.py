...
class ConservativeInnerDiskODE(AdvectiveDisk):
    def __init__(self, *args, name="Inner Disk With Outflows", H_in=0, **kwargs):
        super().__init__(*args, name=name, **kwargs)
        self.H_in = H_in
        # self.solve()
    ...
    def ode(self, x, y):
        ...
        # print("H is zero", np.where(H == 0))
        # H[0] = 1e-7
        dWrphi = self._torque_derivative(self.Mdot_0, Wrphi, R)
        dH = self.Hprime_simplified(self.Mdot_0, H, R, Wrphi, dWrphi, w)
    ...
    def solve(self, **kwargs):
        ...
        print(H_guess / self.R)
        Wrphi_guess = self.torque(self.R)
        Wrphi_guess[0] = self.Wrphi_in
        # self.H_out = H_guess[-1]
        # H_guess[0] = self.H_in  # boundary condition, H=0 at the inner radius

        initial_guess = np.array([H_guess, Wrphi_guess])
        ...
        self.Qrad = self.Q_rad(self.H)
        self.Qvis = self.Q_vis(self.Wrphi)
        dWrphi = self._torque_derivative(self.Mdot_0, self.Wrphi, self.R)
